...
const BrandManagementPage = () => {
  ...
  const logoMutation = useMutation({
    mutationFn: async (params: FormData) => {
      return await fetch(
        [ENV.API_BASE_URL, "api", "brand", selectedBrand, "logo"].join("/"),
        {
          credentials: "include",
          body: params,
          method: "put",
  ...
  return (
    <Space direction={"vertical"}>
      ...
      <Modal
        confirmLoading={logoMutation.isLoading}
        open={logoModal}
        onOk={() => setLogoModal(false)}
        onCancel={() => setLogoModal(false)}
        footer={null}
        title={t("FORM.LOGO")}
      >
        <Form
          ...
          onFinish={(values) => {
            ...
            formData.set("Logo", values["logo"]?.at(0).originFileObj);

            logoMutation.mutate(formData);
          }}
          disabled={logoMutation.isLoading}
        >
          <Form.Item
            name={"logo"}
          ...
          >
            <Upload.Dragger multiple={false} maxCount={1} name={"logo"}>
          ...
          <Form.Item>
            <Space>
              <Button onClick={() => logoForm.resetFields()}>
                {t("FORM.LOGO.RESET")}
              </Button>
              <Button type={"primary"} htmlType={"submit"}>
                {t("FORM.LOGO.SUBMIT")}
